...
interface Props {
  /** 용지 크기: "A4" 또는 "A3" */
  pageSize?: string;
  /** 콘텐츠 확대 배율 (A4=1, A3=√2≈1.4142) */
  scale?: number;
}

export default function QrSheet({ pageSize = "A4", scale = 1 }: Props) {
  return (
    <>
      <PrintGlobal $size={pageSize} />
      <Sheet $scale={scale}>
        <Title>까꿍 후원</Title>
        <VerseLine>
...
const PrintGlobal = createGlobalStyle<{ $size: string }>`
  @page { size: ${(p) => p.$size} portrait; margin: 0; }
  @media print {
    html, body { background: #f6efe2 !important; }
  }
`;

const Sheet = styled.div<{ $scale: number }>`
  width: 210mm;
  min-height: 297mm;
  zoom: ${(p) => p.$scale};
  margin: 0 auto;
